...
class Model:
    ...
    def load_model(self):
        ...
        artifact_uri = f"runs:/{model_version.run_id}/preprocessing"

        pipeline_path = mlflow.artifacts.download_artifacts(
            artifact_uri=artifact_uri,
            dst_path="tmp_artifacts"
        )
        ...
        pipeline_file = os.path.join(pipeline_path, "transform_pipeline.pkl")

        self.model = mlflow.sklearn.load_model(
            f"models:/{MLFLOW_REGISTRY_NAME}@{alias}"
        )
        # We must also retrieve transform pipeline from artifacts
        self.transform_pipeline = joblib.load(pipeline_file)
